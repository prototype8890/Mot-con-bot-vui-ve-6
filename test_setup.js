...
async function testBananaGun() {
  ...
  const {
    BANANA_GUN_API_URL: apiUrl,
    BANANA_GUN_API_KEY: apiKey,
    BANANA_GUN_WALLET_ID: walletId,
    ...
    BANANA_GUN_AUTO_APPROVE: autoApprove,
    BANANA_GUN_EXTRA_HEADERS: extraHeaders,
    BANANA_GUN_SOURCE: sourceTag,
    BANANA_GUN_SELL_PERCENT: sellPercent,
    BANANA_GUN_SELL_API_URL: sellUrl
  } = process.env;

  const provided = [apiUrl, apiKey, walletId, amountEth].filter(Boolean).length;

  const optionalValues = [slippageBps, priorityFeeGwei, gasMultiplier, autoApprove, extraHeaders, sourceTag, sellUrl];
  ...
  if (!apiUrl || !apiKey || !walletId || !amountEth) {
    warn('Cần đủ BANANA_GUN_API_URL, BANANA_GUN_API_KEY, BANANA_GUN_WALLET_ID, BANANA_GUN_BUY_AMOUNT_ETH để bật auto-buy.');
    ok = false;
  }
  ...
  if (apiUrl) {
    try {
      new URL(apiUrl);
      success(`BANANA_GUN_API_URL: ${apiUrl}`);
    } catch (e) {
      error(`BANANA_GUN_API_URL không hợp lệ: ${e.message}`);
      ok = false;
    }
  }

  if (apiKey) {
    success(`BANANA_GUN_API_KEY: ${apiKey.slice(0, 6)}***${apiKey.slice(-4)}`);
  }

  if (walletId) {
    success(`BANANA_GUN_WALLET_ID: ${walletId}`);
  }
  ...
  if (autoApprove) {
    info(`BANANA_GUN_AUTO_APPROVE: ${autoApprove}`);
  }

  if (sourceTag) {
    info(`BANANA_GUN_SOURCE: ${sourceTag}`);
  }

  if (extraHeaders) {
    try {
      JSON.parse(extraHeaders);
      success('BANANA_GUN_EXTRA_HEADERS: JSON hợp lệ');
    } catch (e) {
      error(`BANANA_GUN_EXTRA_HEADERS không phải JSON hợp lệ: ${e.message}`);
      ok = false;
    }
  }
  ...
  if (sellUrl) {
    try {
      new URL(sellUrl);
      success(`BANANA_GUN_SELL_API_URL: ${sellUrl}`);
    } catch (e) {
      error(`BANANA_GUN_SELL_API_URL không hợp lệ: ${e.message}`);
      ok = false;
    }
  }
